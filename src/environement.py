...
class ContiWorld():
    ...
    def __init__(self, env_params, common_params, visu_params, env_dir, params) -> None:
        ...
        if env_params["generate"] == True:
            ...
            self.params["env"]["goal_loc"] = self.VisuGrid[torch.randint(0,self.VisuGrid.shape[0],(1,)).item()].tolist()
            self.plot2D()
            with open(self.env_dir + "/params_env.yaml", "w", encoding = "utf-8") as yaml_file:
                dump = yaml.dump(self.params["env"], default_flow_style = False, allow_unicode = True, encoding = None)
    ...
    def get_disk_constraint_observation(self, disc_nodes):
        noise = torch.normal(mean=torch.zeros(
            len(disc_nodes), 1), std=torch.ones(len(disc_nodes), 1)*self.Fx_noise)
    ...
    def true_constraint_sampling(self):
        ...
        sample = self.Cx_model.posterior(
            self.VisuGrid).sample().reshape(-1, 1)
    ...
    def get_multi_density_observation(self, sets):
        ...
        train["Fx_Y"] = self.Fx_model_cont.posterior(
            locs.reshape(-1, 2)).mean
    ...
    def get_multi_constraint_observation(self, sets):
        train = {}
        locs = torch.vstack(sets)
        train["Cx_Y"] = self.Cx_model_cont.posterior(
            locs.reshape(-1, 2)).mean
        # TODO may be change it such that it returens a list of 1D tensor
        return train["Cx_Y"]
    ...
    def get_density_observation(self, loc):
        obs_Fx = []
        noise = torch.normal(mean=torch.zeros(
            2, 1), std=torch.ones(2, 1)*self.Fx_noise)
        obs_Fx.append(self.Fx_model_cont.posterior(
            loc.reshape(-1, 2)).mean + noise[0])
        return torch.stack(obs_Fx).reshape(-1, 1)

    def get_constraint_observation(self, loc):
        obs_Cx = []
        noise = torch.normal(mean=torch.zeros(
            2, 1), std=torch.ones(2, 1)*self.Cx_noise)
        # self.Cx_model_cont.posterior(loc.reshape(-1,2)).mean
        obs_Cx.append(self.Cx_model_cont.posterior(
            loc).mean + noise[0])
    ...
    def get_true_lipschitz(self): # get_true_safety_func
        ...
        while tol > 0.001:
            ...
            shape = {key: self.params["env"]["shape"][key]*i for key in self.params["env"]["shape"]}
    ...
    def get_safe_init(self):
        ...
        init_xy["Cx_Y"] = self.get_multi_constraint_observation(
            init_xy["Cx_X"])
        init_xy["Fx_Y"] = self.get_multi_density_observation(init_xy["Fx_X"])
        return init_xy

    def step(self, action_u):
        self.state = torch.matmul(self.A, self.state) + \
            torch.matmul(self.B, action_u)
        return self.state
...
if __name__ == "__main__":
    ...
    if params["env"]["generate"]:
        ...
        env_load_path = workspace + \
            "/experiments/16-07-22-Sat/environments/env_" + str(0) + "/"
        save_path = env_load_path + "/"

        env = ContiWorld(
            env_params=params["env"], common_params=params["common"], visu_params=params["visu"], env_dir=save_path,params=params)
    # env = ContiWorld()
    # for i in range(10):
