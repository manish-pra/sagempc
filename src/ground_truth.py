...
from src.central_graph import CentralGraph, expansion_operator
...
class GroundTruth():
    ...
    def __load_opt_graphs(self):
        if 's_opt' in self.env_data:
            ...
            for agent_i in self.env.env_data["s_opt"]:
                self.optimal_graphs_eps.append(
                    self.env.env_data["s_opt"][agent_i])
        else:
            ...
            for i in range(self.params["env"]["n_players"]):
                ...
                if self.params["agent"]["use_goose"]:
                    out = expansion_operator(optimal_graph.graph,  self.true_constraint_function - self.params["common"]["epsilon"], self.init_safe["idx"][i],
                                             self.params["common"]["constraint"], self.params["agent"]["Lc"])
                else:
                    out = optimal_graph.graph.copy()
                self.optimal_graphs_eps.append(out)
        Rbar_eps_nodes_union = set.union(*[set(self.optimal_graphs_eps[i].nodes)
                                           for i in range(self.params["env"]["n_players"])])
    ...
    def compute_normalization_factor(self):
        ...
        for i in range(self.params["env"]["n_players"]):
            # Safe.append(SafeSet(self.init_safe[i], self.init_safe[i], V))
            if self.params["agent"]["use_goose"]:
                out = expansion_operator(optimal_graph.graph,  self.true_constraint_function, self.init_safe["idx"][i],
                                         self.params["common"]["constraint"], self.params["agent"]["Lc"])
            else:
                out = optimal_graph.graph.copy()
            self.optimal_graphs0.append(out)

        Rbar0_nodes_union = set.union(*[set(self.optimal_graphs0[i].nodes)
                                        for i in range(self.params["env"]["n_players"])])
        self.normalization_Rbar0 = torch.sum(
            self.true_density[list(Rbar0_nodes_union)])
                ...
                for agent_idx in range(lead_idx + 1, len(self.optimal_graphs_eps)):
                    if agent_idx not in skip_list:
                        ...
                        if len(G.nodes) > 0:
                            ...
                            self.true_associate_dict[lead_idx].append(
                                agent_idx)
    ...
    def __compute_opt_soln(self):
        ...
        for key in self.true_associate_dict:
            ...
            idx_x_curr, dist_gain, opt_Fx_obj = greedy_algorithm_opti(
                self.true_density.clone(), self.optimal_graphs_eps[key], k, self.params["common"]["disk_size"])
            end = time.time()
            print("time:", end - start)
            opt_goal_x, opt_goal_y = self.grid_V[idx_x_curr], self.true_density[idx_x_curr]
            self.opt_goal["Fx_X"] = torch.cat(
                [self.opt_goal["Fx_X"], opt_goal_x])
            self.opt_goal["Fx_Y"] = torch.cat(
                [self.opt_goal["Fx_Y"], opt_goal_y])
            self.opt_val += opt_Fx_obj

    def __compute_opt_feas_boundary(self):
        for key in self.true_associate_dict:
            G = self.optimal_graphs_eps[key]
            self.optimal_feasible_boundary[key] = [x for x in G.nodes() if (
                G.out_degree(x) == 3 or G.out_degree(x) == 2)]
    ...
    def multi_coverage_oracle(self, idx_x_list, density, graph, disk_size, key):
        ...
        for idx_x in idx_x_list:
            if idx_x not in graph.nodes:  # The logic is not valid for 2d, ideally give large penalty of being unsafe if this comes up
                if idx_x in self.optimal_graphs0[key].nodes:
                    coverage_area_idx = set(nx.single_source_shortest_path_length(
                        self.optimal_graphs0[key], idx_x, cutoff=disk_size))
                    diff_nodes = set(
                        self.optimal_graphs0[key].nodes) - set(graph.nodes)
                ...
                Fs = coverage_oracle(
                    idx_x, non_covered_density, graph, disk_size)
                non_covered_density[list(nx.single_source_shortest_path_length(
                    graph, idx_x, cutoff=disk_size))] = 0.0
            total_Fs += Fs
        return total_Fs
    ...
    def compute_cover_xIX_rho_Rbar(self, players, graph_type):
        ...
        for key in self.true_associate_dict:
            ...
            xn_buddies = [players[agent_idx].planned_disk_center
                          for agent_idx in self.true_associate_dict[key]]  # regret of measure pt and regret of planned pt. Only problem is that planned pt can also be outside of safe opt and then calculation can be an issue
            # idx_star = [
            ...
            if graph_type == 'eps':
                Fx_obj_at_diff_density = self.multi_coverage_oracle(
                    idx_xn_buddies, self.true_density, self.optimal_graphs_eps[key], self.params["common"]["disk_size"], key)
            else:
                Fx_obj_at_diff_density = self.multi_coverage_oracle(
                    idx_xn_buddies, self.true_density, self.optimal_graphs0[key], self.params["common"]["disk_size"], key)
    ...
    def compute_cover_xIX_lcb_pessi(self, players):
        ...
        for key in self.true_associate_dict:
            # xn_buddies are the agents which are in same constrain set
            xn_buddies = [players[agent_idx].planned_disk_center_at_last_meas
                          for agent_idx in self.true_associate_dict[key]]  # regret of measure pt and regret of planned pt. Only problem is that planned pt can also be outside of safe opt and then calculation can be an issue
            # idx_star = [
            #     players[self.true_associate_dict[key][0]].get_idx(xn_buddies)]
            # Fx_obj_at_diff_density = objective_oracle_given(
            ...
            for xn in xn_buddies:
                idx_xn_buddies.append(idxfromloc(self.grid_V, xn))
            Fx_obj_at_diff_density = self.multi_coverage_oracle(
                idx_xn_buddies, players[key].get_lcb_density(), pessimistic_graphs[key], self.params["common"]["disk_size"], key)
            sum_Fx_obj += Fx_obj_at_diff_density
    ...
    def compute_cover_xlIX_rho_pessi(self, players, pessi_associate_dict):
        ...
        for key in pessi_associate_dict:
            ...
            idx_xn_star, acq_density, M_dist, Fx_obj = players[key].get_lcb_pessi_coverage_point(
                k)
        ...
        for key in self.true_associate_dict:
            # xn_buddies are the agents which are in same constrain set
            idx_xn_buddies = [players[agent_idx].set_greedy_lcb_pessi_goal
    ...
    def compute_cover_xIX_rho_opti(self, players, associate_dict):
        ...
        for key in associate_dict:
            # xn_buddies are the agents which are in same constrain set
            xn_buddies = [players[agent_idx].get_recommendation_pt()
                          for agent_idx in associate_dict[key]]  # regret of measure pt and regret of planned pt. Only problem is that planned pt can also be outside of safe opt and then calculation can be an issue
            # idx_star = [
            ...
            Fx_obj_at_diff_density = self.multi_coverage_oracle(
                idx_xn_buddies, self.true_density, optimistic_graphs[key], self.params["common"]["disk_size"], key)
            sum_Fx_obj += Fx_obj_at_diff_density
        return sum_Fx_obj
        ...
        for key, player in enumerate(players):
            for asso_key in opti_associate_dict:
                if key in opti_associate_dict[asso_key]:
                    bool_condi_agents = torch.Tensor(
                        opti_associate_dict[asso_key]) < key
                    condi_agents = torch.IntTensor(opti_associate_dict[asso_key])[
                        bool_condi_agents].tolist()
                    # xn_buddies are the agents which are in same constrain set
                    xn_buddies = [players[agent_idx].get_recommendation_pt()
                                  for agent_idx in condi_agents]  # regret of measure pt and regret of planned pt. Only problem is that planned pt can also be outside of safe opt and then calculation can be an issue
                    idx_xn_buddies = []
                    for xn in xn_buddies:
                        idx_xn_buddies.append(idxfromloc(self.grid_V, xn))
        ...
        for key, player in enumerate(players):
            for asso_key in self.true_associate_dict:
                if key in self.true_associate_dict[asso_key]:
                    bool_condi_agents = torch.Tensor(
                        self.true_associate_dict[asso_key]) < key
    ...
    def compute_opt_greedy_given_others(self, key, others, optimistic_graphs):
        non_covered_density = torch.empty_like(
            self.true_density).copy_(self.true_density)
        ...
        for idx_x_curr in others:
            ...
            if idx_x_curr in optimistic_graphs[key].nodes:
                non_covered_density[list(nx.single_source_shortest_path_length(
                    optimistic_graphs[key], idx_x_curr, cutoff=self.params["common"]["disk_size"]))] = 0.0

        idx_max_gain, M_dist, max_gain = greedy_algorithm(
            non_covered_density, optimistic_graphs[key], 1, self.params["common"]["disk_size"])
